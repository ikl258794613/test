...
router.get("/", async (req, res) => {
  // const productresults = await connection.queryAsync("SELECT * FROM official_product");//所有官方商品資料
  const count = await connection.queryAsync(
    "SELECT COUNT(*) as total FROM official_product WHERE volume=?",
    ["50ml"]
  );
  
  // "SELECT * FROM official_product WHERE series_id=1 " 茶系列
  // "SELECT * FROM official_product WHERE series_id=2 " 動物系列
  // "SELECT * FROM official_product WHERE series_id=3 " 自然系列
  // "SELECT * FROM official_product WHERE series_id=4 " 酒系列
  // "SELECT * FROM official_product " 全系列
